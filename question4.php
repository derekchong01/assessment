...
function roll_rounds($num_rolls = 100) {
    ...
    foreach ($vip_rank as $rank) {
        // Initialize item counts for this VIP rank with zero counts for each rarity
        $results[$rank] = array_fill_keys($item_tier_rarity, 0);

        // Perform the specified number of item rolls
        for ($i = 0; $i < $num_rolls; $i++) {
            $item = roll_item($rank); // Get the rolled item
            $results[$rank][$item]++; // Increment the count for the rolled item
        }
    }
